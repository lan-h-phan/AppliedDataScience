...
##Chapter 4-6: at the end of chapter 4, 5, and 6, provide written text, code, and the visualization(s) 
#that address the “where to go next” issues at the end of each of these chapters.

##Chapter 4.8 Where to go next
...
gss_sm %>%
  ...
  geom_smooth() +
  facet_grid(sex~race)
print("This plot treats/ presents 2 separate variables") 
...
gss_sm %>%
  ...
  geom_smooth() +
  facet_wrap(sex ~ race)
#Like facet_grid(), the facet_wrap() function can facet on two or more variables at once. But it will do it by laying the results out in a wrapped one-dimensional table instead of a fully cross-classified grid.
#facet_wrap treats the variables similar to when we use facet_grid(~sex + race)
...
#it is less clustered without geom_point, but it was helpful to see how the density lines were created


##Chapter 5.7 Where to go next
...
#p4 You will need to filter some rows, group the data by continent, 
#and calculate the mean life expectancy by continent before beginning the plotting process.
gapminder %>%
  filter(year == 2007) %>%
  group_by(continent) %>%
...
#p5
gss_sm %>% group_by(religion, degree) %>%
  summarize(N = n()) %>%
  mutate(pct = round(N / sum(N)*100, 0)) 

gss_sm %>% group_by(sex, degree) %>%
  summarize(N = n()) %>%
  mutate(pct = round(N / sum(N)*100, 0)) 
...
dat_small %>%
  ...
  labs(x = "Socio-economic Status",
       y = "Math Score")
print("The relationship between ESCS and math does not change across gender and regions 
      - it is a positive correlation across groups. So this means the lower their socio-economic status,
      the lower their math scores.")
